...
const TiltCard = ({ icon, title, description, children }: { icon?: React.ReactNode, title?: string, description?: string, children?: React.ReactNode }) => {
  ...
  return (
    <motion.div
      ...
      className="relative bg-gradient-to-br from-gray-800/60 via-gray-800/40 to-gray-900/60 backdrop-blur-xl p-8 rounded-3xl border border-white/10 shadow-2xl hover:shadow-blue-500/20 hover:border-blue-400/50 group overflow-hidden h-full"
    >
  ...
  <SectionWrapper id="features" className="bg-black/20">
    <div className="container mx-auto">
      ...
      <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-10">
        <TiltCard icon={<Cpu size={32} />} title="AI-Powered Dashboard" description="Get instant insights with a smart dashboard that calculates your productivity score." />
        <TiltCard icon={<Zap size={32} />} title="Advanced Task Management" description="Organize your academic life with a robust task manager. Prioritize and never miss a deadline." />
        <TiltCard icon={<BarChart size={32} />} title="In-Depth Analytics" description="Visualize your study habits, track grade distribution, and identify strengths and weaknesses." />
        <TiltCard icon={<BookOpen size={32} />} title="Comprehensive Course Hub" description="Manage all your courses, from materials to schedules, in one centralized location." />
        <TiltCard icon={<Calendar size={32} />} title="Smart Timetable" description="Stay on top of your schedule with an intelligent timetable that shows upcoming classes." />
        <TiltCard icon={<Shield size={32} />} title="Zero-Trust Security" description="Your data is protected with cutting-edge security, including device fingerprinting." />
      </div>
    </div>
  </SectionWrapper>
);
...
export const Pricing = () => {
  const plans = [
    { 
      ...
      price: "₹0", 
      period: "forever", 
      features: ["Basic Timetable", "Task Manager", "Grade Tracker", "5GB Storage", "Email Support"], 
      cta: "Get Started", 
      popular: false, 
      link: "/auth"  
    },
          ...
          {plans.map((plan, index) => (
            <TiltCard key={index}>
              <div className="relative">
                {plan.popular && (
                  <div className="absolute -top-12 left-1/2 transform -translate-x-1/2 bg-gradient-to-r from-blue-500 to-purple-600 text-white px-6 py-1 rounded-full text-sm font-bold shadow-lg">
                    Most Popular
                  </div>
                )}
                ...
                <Link to={plan.link}>
                  <button className={`w-full py-4 rounded-xl font-bold text-lg transition-all ${plan.popular ? 'bg-gradient-to-r from-blue-500 to-purple-600 text-white hover:shadow-lg hover:shadow-blue-500/50' : 'bg-white/10 text-white hover:bg-white/20'}`}>
                    {plan.cta}
                  </button>
                </Link>
              </div>
